...
import cn.osc.gitee.api.data.request.Type
...
import cn.osc.gitee.ui.GiteeShareDialog
...
class GiteeShareAction : DumbAwareAction(GiteeBundle.messagePointer("share.action"),
                                          GiteeBundle.messagePointer("share.action.description"),
  ...
  companion object {
    ...
    fun shareProjectOnGithub(project: Project, file: VirtualFile?) {
      ...
      val accountInformationLoader = object : (GiteeAccount, Component) -> Pair<Boolean, Set<String>> {
        ...
        override fun invoke(account: GiteeAccount, comp: Component) = loadedInfo.getOrPut(account) {
          ...
          while (true) {
            try {
              return progressManager.runProcessWithProgressSynchronously(ThrowableComputable<Pair<Boolean, Set<String>>, IOException> {
                ...
                val requestExecutor = GiteeApiRequestExecutor.Factory.getInstance().create(token)
                ...
                val names = GiteeApiPagesLoader
                  .loadAll(requestExecutor, progressManager.progressIndicator,
                           GiteeApiRequests.CurrentUser.Repos.pages(account.server, Type.OWNER))
                  .mapSmartSet { it.name }
              ...
              GEAccountsUtil.requestNewToken(account, project, comp) ?: throw mte
            }
          }
        }
      }

      val shareDialog = GiteeShareDialog(project,
      ...
      object : Task.Backgroundable(project, GiteeBundle.message("share.process")) {
        ...
        override fun run(indicator: ProgressIndicator) {
          val token = GECompatibilityUtil.getOrRequestToken(account, project) ?: return
          ...
          url = requestExecutor
            .execute(indicator, GiteeApiRequests.CurrentUser.Repos.create(account.server, name, description, isPrivate)).htmlUrl
          ...
          indicator.text = GiteeBundle.message("share.process.pushing.to.github.master")
          if (!pushCurrentBranch(project, repository, remoteName, remoteUrl, name, url)) {
            return
          }

          GiteeNotifications.showInfoURL(project,
                                          GiteeNotificationIdsHolder.SHARE_PROJECT_SUCCESSFULLY_SHARED,
                                          GiteeBundle.message("share.process.successfully.shared"),
                                          name,
                                          url)
        }
        ...
        private fun performFirstCommitIfRequired(project: Project,
          ...
          try {
            ...
            if (!dialog.isOK || files2commit.isEmpty()) {
              GiteeNotifications.showInfoURL(project,
                                              GiteeNotificationIdsHolder.SHARE_EMPTY_REPO_CREATED,
                                              GiteeBundle.message("share.process.empty.project.created"),
                                              name, url)
              return false
            }
            ...
            GiteeNotifications.showErrorURL(project, GiteeNotificationIdsHolder.SHARE_PROJECT_INIT_COMMIT_FAILED,
                                             ...
                                             " '$name' ",
                                             GiteeBundle.message("share.error.init.commit.failed") + GiteeUtil.getErrorTextFromException(
                                               e),
                                             url)
            return false
          }
  ...
  class GiteeExistingRemotesDialog(project: Project, private val remotes: List<String>) : DialogWrapper(project) {
    init {
      title = GiteeBundle.message("share.error.project.is.on.github")
      setOKButtonText(GiteeBundle.message("share.anyway.button"))
      init()
    }

    override fun createCenterPanel(): JComponent {
      val mainText = JBLabel(if (remotes.size == 1) GiteeBundle.message("share.action.remote.is.on.github")
                             else GiteeBundle.message("share.action.remotes.are.on.github"))
