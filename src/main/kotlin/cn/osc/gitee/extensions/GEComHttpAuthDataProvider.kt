...
import cn.osc.gitee.api.GiteeServerPath.DEFAULT_SERVER
...
private suspend fun getAuthDataOrCancel(project: Project, url: String, login: String?): AuthData {
  ...
  return withContext(Dispatchers.EDT + ModalityState.any().asContextElement()) {
    when (accountsWithTokens.size) {
      ...
      1 -> GEAccountsUtil.requestReLogin(accountsWithTokens.keys.first(), project = project)
      else -> GESelectAccountHttpAuthDataProvider(project, accountsWithTokens).getAuthData(null)
    }
