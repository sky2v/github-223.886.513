...
class CachingGHUserAvatarLoader : Disposable {
  ...
  fun requestAvatar(requestExecutor: GiteeApiRequestExecutor, url: String): CompletableFuture<Image?> = avatarCache.get(url) {
    ProgressManager.getInstance().submitIOTask(indicatorProvider) { loadAndDownscale(requestExecutor, it, url, STORED_IMAGE_SIZE) }
  }

  private fun loadAndDownscale(requestExecutor: GiteeApiRequestExecutor, indicator: ProgressIndicator,
                               url: String, maximumSize: Int): Image? {
    try {
      val loadedImage = requestExecutor.execute(indicator, GiteeApiRequests.CurrentUser.getAvatar(url))
