...
import cn.osc.gitee.authentication.GiteeAuthenticationManager
...
object GiteeUtil {
  ...
  @NlsSafe
  const val SERVICE_DISPLAY_NAME: String = "Gitee Scan Demo"
  ...
  @JvmStatic
  @Deprecated("{@link GiteeGitHelper}", ReplaceWith("GiteeGitHelper.findGitRepository(project, file)",
                                                     "cn.osc.gitee.util.GiteeGitHelper"))
  @ApiStatus.ScheduledForRemoval
  fun getGitRepository(project: Project, file: VirtualFile?): GitRepository? {
    return GiteeGitHelper.findGitRepository(project, file)
  }

  @JvmStatic
  @Deprecated("{@link GiteeGitHelper}")
  ...
  @JvmStatic
  @Deprecated("{@link cn.osc.gitee.api.GiteeServerPath}, {@link GiteeGitHelper}")
  private fun findGithubRemote(repository: GitRepository): Pair<GitRemote, String>? {
    val server = GiteeAuthenticationManager.getInstance().getSingleOrDefaultAccount(repository.project)?.server ?: return null
  ...
  @JvmStatic
  @Deprecated("{@link cn.osc.gitee.api.GiteeServerPath}")
  @ApiStatus.ScheduledForRemoval
