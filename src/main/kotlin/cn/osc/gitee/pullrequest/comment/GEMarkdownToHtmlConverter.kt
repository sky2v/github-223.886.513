...
class GEMarkdownToHtmlConverter(private val project: Project?) {
  ...
  fun convertMarkdown(@NlsSafe markdownText: String): String {
    ...
    val flavourDescriptor = GiteeFlavourDescriptor(CodeBlockHtmlSyntaxHighlighter(project))
  ...
  fun convertMarkdownWithSuggestedChange(suggestedChange: GESuggestedChange): String {
    ...
    val flavourDescriptor = GiteeFlavourDescriptor(htmlSyntaxHighlighter)

    return MarkdownToHtmlConverter(flavourDescriptor).convertMarkdownToHtml(text, null)
  }

  private class GiteeFlavourDescriptor(
