...
import cn.osc.gitee.pullrequest.data.GEPRSearchQuery
import cn.osc.gitee.pullrequest.data.GEPRSearchQuery.QualifierName
import cn.osc.gitee.pullrequest.data.GEPRSearchQuery.Term
import cn.osc.gitee.pullrequest.data.GEPRSearchQuery.Term.Qualifier
import cn.osc.gitee.pullrequest.data.GEPRSearchQuery.Term.QueryPart

@Serializable
internal data class GEPRListSearchValue(override val searchQuery: String? = null,
  ...
  fun toQuery(): GEPRSearchQuery? {
    ...
    if (state != null) {
      val term = when (state) {
        State.OPEN -> Qualifier.Enum(QualifierName.`is`, GiteeIssueState.open)
        State.CLOSED -> Qualifier.Enum(QualifierName.`is`, GiteeIssueState.closed)
    ...
    return GEPRSearchQuery(terms)
  }

  companion object {
    val DEFAULT = GEPRListSearchValue(state = State.OPEN)
    val EMPTY = GEPRListSearchValue()
  }
