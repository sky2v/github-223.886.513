...
import cn.osc.gitee.api.data.request.GiteeGistRequest.FileContent;
import cn.osc.gitee.authentication.accounts.GiteeAccount;
import cn.osc.gitee.i18n.GiteeBundle;
import cn.osc.gitee.ui.GiteeCreateGistDialog;
  ...
  private static void createGistAction(@NotNull final Project project,
    ...
    new Task.Backgroundable(project, GiteeBundle.message("create.gist.process")) {
      ...
      public void run(@NotNull ProgressIndicator indicator) {
        String token = GECompatibilityUtil.getOrRequestToken(account, project);
        if (token == null) return;
        GiteeApiRequestExecutor requestExecutor = GiteeApiRequestExecutor.Factory.getInstance().create(token);
