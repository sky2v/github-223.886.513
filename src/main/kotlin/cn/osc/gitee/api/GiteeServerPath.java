...
/**
 * Gitee server reference allowing to specify custom port and path to instance
 */
@Tag("server")
public final class GiteeServerPath implements ServerPath {
  public static final String DEFAULT_HOST = "github.com";
  public static final GiteeServerPath DEFAULT_SERVER = new GiteeServerPath(DEFAULT_HOST);
  ...
  public GiteeServerPath() {
    this(null, "", null, null);
  }

  public GiteeServerPath(@NonNls @NotNull String host) {
    this(null, host, null, null);
  }

  public GiteeServerPath(@Nullable Boolean useHttp,
  ...
  @NotNull
  public static GiteeServerPath from(@NotNull String uri) throws GiteeParseException {
    Matcher matcher = URL_REGEX.matcher(uri);

    if (!matcher.matches()) throw new GiteeParseException("Not a valid URL");
    String schema = matcher.group(1);
    Boolean httpSchema = (schema == null || schema.isEmpty()) ? null : schema.equalsIgnoreCase("http://");
    String host = matcher.group(2);
    if (host == null) throw new GiteeParseException("Empty host");
    ...
    if (portGroup == null) {
      ...
      try {
        ...
        throw new GiteeParseException("Invalid port format");
      }
    }

    String path = StringUtil.nullize(matcher.group(5));

    return new GiteeServerPath(httpSchema, host, port, path);
  }
  ...
  public boolean equals(Object o, boolean ignoreProtocol) {
    ...
    if (!(o instanceof GiteeServerPath)) return false;
    GiteeServerPath path = (GiteeServerPath)o;
