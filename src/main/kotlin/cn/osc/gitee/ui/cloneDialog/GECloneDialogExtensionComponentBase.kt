...
internal abstract class GECloneDialogExtensionComponentBase(
  ...
  init {
    repositoryList = JBList(loader.listModel).apply {
      cellRenderer = GERepositoryListCellRenderer(ErrorHandler()) { accountListModel.itemsSet }
      isFocusable = false
  ...
  protected fun getAccounts(): Set<GiteeAccount> = accountListModel.itemsSet
  ...
  private fun getGithubRepoPath(searchText: String): GERepositoryCoordinates? {
    val url = searchText
      .trim()
  ...
  protected abstract fun createAccountMenuLoginActions(account: GiteeAccount?): Collection<AccountMenuItem.Action>
