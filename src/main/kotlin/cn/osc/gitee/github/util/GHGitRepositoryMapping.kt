...
class GHGitRepositoryMapping(override val repository: GHRepositoryCoordinates,
  ...
  val gitRemoteUrlCoordinates: cn.osc.gitee.github.util.GitRemoteUrlCoordinates = GitRemoteUrlCoordinates(remote)
  ...
  companion object {
    ...
    fun create(server: GithubServerPath, remote: cn.osc.gitee.github.util.GitRemoteUrlCoordinates): GHGitRepositoryMapping? {
      val repositoryPath = GithubUrlUtil.getUserAndRepositoryFromRemoteUrl(remote.url) ?: return null
      val repository = GHRepositoryCoordinates(server, repositoryPath)
